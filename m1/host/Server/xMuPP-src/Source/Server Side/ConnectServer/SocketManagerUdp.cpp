...
bool CSocketManagerUdp::DataRecv() // OK
{
	...
	while(true)
	{
		...
		{
			// Season 2 stock clients request server list over UDP (C1:F4:06 / C1:F4:03).
			// TCP ConnectServerProtocol already handles the same on the TCP port.
			if(head == 0xF4)
			{
				BYTE subh = lpMsg[count+3];
				...
				if(subh == 0x06)
				{
					...
					int msgSize = sizeof(pMsg);
					int serverCount = gServerList.GenerateServerList(send,&msgSize);
					pMsg.count[0] = SET_NUMBERHB(serverCount);
					pMsg.count[1] = SET_NUMBERLB(serverCount);
					pMsg.header.size[0] = SET_NUMBERHB(msgSize);
					pMsg.header.size[1] = SET_NUMBERLB(msgSize);
					memcpy(send,&pMsg,sizeof(pMsg));
					this->DataSendTo(&this->m_ClientAddr,send,msgSize);
					LogAdd(LOG_BLUE,"[SocketManagerUdp] ServerList -> %s:%d (count=%d)",inet_ntoa(this->m_ClientAddr.sin_addr),ntohs(this->m_ClientAddr.sin_port),serverCount);
				}
				else if(subh == 0x03)
				{
					PMSG_SERVER_INFO_RECV* lpInfo = (PMSG_SERVER_INFO_RECV*)&lpMsg[count];
					if(gServerList.CheckJoinServerState() != 0)
					{
						SERVER_LIST_INFO* lpServerListInfo = gServerList.GetServerListInfo(lpInfo->ServerCode);
						if(lpServerListInfo != 0 && lpServerListInfo->ServerShow != 0 && lpServerListInfo->ServerState != 0)
						{
							...
							this->DataSendTo(&this->m_ClientAddr,(BYTE*)&pMsg,pMsg.header.size);
							LogAdd(LOG_BLUE,"[SocketManagerUdp] ServerInfo(%d) -> %s:%d",lpInfo->ServerCode,inet_ntoa(this->m_ClientAddr.sin_addr),ntohs(this->m_ClientAddr.sin_port));
						}
					}
				}
			}
...
bool CSocketManagerUdp::DataSendTo(SOCKADDR_IN* lpAddr,BYTE* lpMsg,int size) // OK
{
	if(this->m_socket == INVALID_SOCKET || lpAddr == 0)
	{
		return 0;
	}

	int result = sendto(this->m_socket,(char*)lpMsg,size,0,(sockaddr*)lpAddr,sizeof(SOCKADDR_IN));

	if(result == SOCKET_ERROR)
	{
		if(WSAGetLastError() != WSAEWOULDBLOCK)
		{
			LogAdd(LOG_RED,"[SocketManagerUdp] sendto(client) failed with error: %d",WSAGetLastError());
			return 0;
		}
	}
...
DWORD WINAPI CSocketManagerUdp::ServerRecvThread(CSocketManagerUdp* lpSocketManagerUdp) // OK
{
	while(true)
	{
		...
		lpSocketManagerUdp->m_ClientAddr = SocketAddr;
